...
def plot_threshold_subtraction(compiled: CompiledIntegrand, x_lim, y_lim, x_axis = None, y_axis = None,  res = 300):
    ...
    plt.subplot(2,3,1)
    integrand = (compiled.eval_integrand(ks_plane)*ks_plane_jac).reshape(res,res)
    plot_complex_plane(xs_plane, integrand)
    plt.subplot(2,3,2)
    counter_term = (compiled.eval_counterterm(ks_plane)*ks_plane_jac).reshape(res,res)
    plot_complex_plane(xs_plane, counter_term)
    plt.subplot(2,3,3)
    subtracted = (compiled.eval_subtracted(ks_plane)*ks_plane_jac).reshape(res,res)
    plot_complex_plane(xs_plane, subtracted)
    plt.subplot(2,3,4)
    plot_complex(x, compiled.eval_integrand(ks_line) * ks_line_jac)
    plt.subplot(2,3,5)
    plot_complex(x, compiled.eval_counterterum_with_jac(ks_line))
    plt.subplot(2,3,6)
    plot_complex(x, compiled.eval_subtracted(ks_line) * ks_line_jac)
    plt.show()
