...
pub fn get_reference_value(args: IntegrandArgs) -> Complex64 {
...
pub fn get_args() -> Vec<IntegrandArgs> {
...
pub fn get_args_result_pairs() -> Vec<(IntegrandArgs, Complex64)> {
    let args = get_args();
    args.into_iter()
        .map(|arg| {
...
#[allow(dead_code)]
pub fn get_real_args_result_pairs() -> Vec<(IntegrandArgs, f64)> {
    get_args_result_pairs()
        .into_iter()
        .filter_map(|arg| {if arg.0.is_real() {Some((arg.0, arg.1.re))} else {None}})
        .collect()
}